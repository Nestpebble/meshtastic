...
:::info
The client role also affects whether messages are sent to the MQTT server. `client_mute` in particular will not work, as it is set specifically not to forward messages. If the same functionality is required with MQTT enabled, the role can be set to `client` and Lora TX can be disabled.
:::
